...
public class Dl4jSeq2SeqUserCode implements TcDeepLearning4jUser {
	...
	public void run(File trainVec, File trainOutcome, File testVec, File testOutcome, File embedding, int seed, File prediction)
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(seed)
				...
				.updater(Updater.SGD).weightInit(WeightInit.XAVIER_UNIFORM)
				.learningRate(learningRate).list()
				.layer(0, new GravesBidirectionalLSTM.Builder().activation(Activation.TANH).nIn(featuresSize).nOut(100).build())
				.layer(1,
						new RnnOutputLayer.Builder().activation(Activation.SOFTMAX)
								.lossFunction(LossFunctions.LossFunction.MCXENT).nIn(100).nOut(maxTagsetSize).build())
		...
		for (int i = 0; i < epochs; i++) {
			...
			Date date = new Date();
			System.out.print("finished " + dateFormat.format(date)+"\n");
			
			System.out.println(String.format("Trained %d minutes\nAcc: %.2f on test set\n", ((end - start)/1000/60),
					computeAccuracy(iTest, mln)));
			iTest.reset();
		}
		
		System.out.println(String.format("Training took: %d hours", (trainTime/1000/60/60)));
	...
	private double computeAccuracy(DataSetIterator iTest, MultiLayerNetwork mln) {
		...
		while (iTest.hasNext()) {
			...
			INDArray outMask = t.getLabelsMaskArray();
			INDArray p = mln.output(features, false);
